...
class FileWatcherApp:
    def __init__(self, root):
        ...
        self.create_gui()
        self.load_config()  # Verplaats het laden van configuratie naar hier
    ...
    def create_gui(self):
        ...
        # Na het laden van de configuratie, roep de update_treeview aan
        self.load_config()
    ...
    def remove_watcher(self):
        ...
        if selected_item:
            ...
            self.update_treeview()
            self.save_config()  # Sla de configuratie onmiddellijk op na verwijderen
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, 'r') as file:
                    ...
                    # Start de observers op basis van de opgeslagen status
                    for i, watcher in enumerate(self.watchers, start=1):
                        status = watcher.get('status', 'stopped')
                        folder = watcher['folder']

                        if status == 'started':
                            self.start_observer(i - 1)  # Index is nul-gebaseerd, daarom i - 1

                        # Update de status van de watchers in de GUI
                        self.tree.insert('', 'end', iid=str(i), text=f"Watcher {i}", values=(status, folder))
