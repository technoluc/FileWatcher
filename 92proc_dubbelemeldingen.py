...
class FileChangeHandler(FileSystemEventHandler):
    def __init__(self, app):
        self.app = app
        self.last_notification = {}  # Een dictionary om bij te houden wanneer een melding voor het laatst is weergegeven
    ...
    def on_modified(self, event):
        ...
        # Controleer of de melding al eerder is weergegeven binnen een bepaalde tijdsperiode (bijv. 1 seconde)
        if src_path not in self.last_notification or (current_time - self.last_notification[src_path]) > 1:
            self.app.show_notification(f"Bestand gewijzigd: {src_path}")
            self.last_notification[src_path] = current_time
